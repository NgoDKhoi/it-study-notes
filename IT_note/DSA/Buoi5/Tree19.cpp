...
double AverageByLevel(TREE root, int level) {
    ...
    while (!currentNodes.empty()) {
        ...
        for (int i = 0; i < currentNodes.size(); i++) {
            if (currentNodes[i]->left != NULL) nextNodes.push_back(currentNodes[i]->left);
            if (currentNodes[i]->right != NULL) nextNodes.push_back(currentNodes[i]->right);
        }
